...
// Chuyển s3://bucket/key thành HTTPS để trình duyệt có thể tải trực tiếp
const convertS3ToHttpUrl = (url) => {
...
const LiveCard = ({ item }) => {
  const videoUrlRaw =
    item?.LivestreamVideoSignedUrl || item?.LivestreamVideoUrl;
  const videoUrl = useMemo(
    () => convertS3ToHttpUrl(videoUrlRaw),
    [videoUrlRaw]
  );
  const [thumbnail, setThumbnail] = useState(item.thumbnail || null);

  // Nếu không có thumbnail, cố gắng lấy một frame bất kỳ trong video
  useEffect(() => {
    ...
    const handleLoadedMetadata = () => {
      const target = video.duration
        ? Math.max(Math.min(video.duration * 0.2, video.duration - 0.5), 0)
        : 0;
  ...
  const displayThumb =
    thumbnail || "https://distenda.vn/default-live-thumbnail.jpg";
      ...
      <div className="flex items-center gap-3 mb-[1.5rem]">
        ...
        <div className="flex-1">
          ...
          <p className="max-md:text-[14px] md:text-[1rem] text-white font-regular">
            {new Date(item.LivestreamStartedAt).toLocaleDateString("vi-VN", {})}
          </p>
        </div>
      </div>
      {videoUrl ? (
          ...
          <div className="flex items-center gap-1">
            ...
            >
              ...
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M16.1537 7.62429C15.7667 7.62429 15.4387 7.32729 15.4067 6.93529C15.3407 6.11329 14.7907 5.41929 14.0077 5.16629C13.6127 5.03829 13.3967 4.61529 13.5237 4.22229C13.6527 3.82829 14.0717 3.61429 14.4677 3.73829C15.8307 4.17929 16.7857 5.38629 16.9027 6.81329C16.9357 7.22629 16.6287 7.58829 16.2157 7.62129C16.1947 7.62329 16.1747 7.62429 16.1537 7.62429"
                fill="white"
              />
            </svg>
          ...
          <div className="flex items-center gap-1">
            ...
            >
              ...
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M14.6964 12.1621C14.1444 12.1621 13.6924 11.7151 13.6924 11.1621C13.6924 10.6091 14.1354 10.1621 14.6874 10.1621H14.6964C15.2484 10.1621 15.6964 10.6091 15.6964 11.1621C15.6964 11.7151 15.2484 12.1621 14.6964 12.1621"
                fill="white"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M10.6876 12.1621C10.1356 12.1621 9.68359 11.7151 9.68359 11.1621C9.68359 10.6091 10.1256 10.1621 10.6786 10.1621H10.6876C11.2396 10.1621 11.6876 10.6091 11.6876 11.1621C11.6876 11.7151 11.2396 12.1621 10.6876 12.1621"
                fill="white"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M6.67832 12.1621C6.12632 12.1621 5.67432 11.7151 5.67432 11.1621C5.67432 10.6091 6.11732 10.1621 6.66932 10.1621H6.67832C7.23032 10.1621 7.67832 10.6091 7.67832 11.1621C7.67832 11.7151 7.23032 12.1621 6.67832 12.1621"
                fill="white"
              />
            </svg>
